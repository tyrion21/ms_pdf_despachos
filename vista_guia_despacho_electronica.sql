...
-- =============================================
-- FUNCIÓN PARA GUÍA DE DESPACHO ELECTRÓNICA CON PARÁMETRO DINÁMICO
-- =============================================
CREATE FUNCTION dbo.FN_GUIA_DESPACHO_ELECTRONICA
(
...
(
    WITH CTE_PARAMETROS AS (
        ...
        SELECT 
            ...
            (SELECT TOP (1) CORRELATIVO_PLAN 
             FROM DESPACHOS 
             WHERE GUIA_DES = CAST(@NUM_FACT AS NVARCHAR(10))
               AND COD_EMP = @COD_EMP
    ...
    CTE_TERMOGRAFOS AS (
        ...
        WHERE T.COD_EMP = P.COD_EMP 
            ...
            AND T.GUIA_DES = CAST(P.NUM_FACT AS NVARCHAR(10))
            AND T.SW_ELEC = 1 
    ...
    CTE_PALLETS AS (
        ...
        SELECT 
            ...
            ISNULL((
                ...
                FROM DESPACHOS 
                WHERE COD_EMP = P.COD_EMP 
                    AND GUIA_DES = CAST(P.NUM_FACT AS NVARCHAR(10))
                    AND SW_ELEC = 1
            ), 0) AS PALLETS
...
/*
-- Ejemplo 1: Consultar guía específica para empresa MER
SELECT * FROM dbo.FN_GUIA_DESPACHO_ELECTRONICA('MER', 72699)

-- Ejemplo 2: Consultar otra guía con valores por defecto para empresa
SELECT * FROM dbo.FN_GUIA_DESPACHO_ELECTRONICA(DEFAULT, 74151)

-- Ejemplo 3: Consultar para otra empresa
SELECT * FROM dbo.FN_GUIA_DESPACHO_ELECTRONICA('OTR', 12345)

-- Ejemplo 4: Usar en un procedimiento almacenado
CREATE PROCEDURE SP_GET_GUIA_ELECTRONICA
    @EMPRESA NVARCHAR(4),
    @NUMERO_FACTURA INT
AS
BEGIN
    SELECT * FROM dbo.FN_GUIA_DESPACHO_ELECTRONICA(@EMPRESA, @NUMERO_FACTURA)
END
*/
